  ...
  useEffect(() => {
    ...
    async function getProjectWithId() {
      try {
        ...
        response?.data?.projectTask.length
          ? setProjectTask(response?.data?.projectTask)
          : setErrMsg("Not any task created yet!");

        response?.data?.projectUser.length
          ? setProjectUser(response?.data?.projectUser)
          : setErrMsg("Not any user listed yet!");
      } catch (err) {
        if (!err?.response) {
          ...
          <div class="table-data">
            <div class="order">
              <div class="head">
                <h3>Tasks</h3>
                {auth?.role === "admin" && (
                  <i class="bx bx-plus" onClick={() => setShowModal(true)}></i>
                )}
                {/* <i class='bx bx-filter'></i> */}
              </div>
              {showModal && <ShowModal closeModal={closeModal} />}
              {showUserModal && (
                <ShowAddUserModel closeUserModel={closeUserModel} />
              )}
              <table>
                <thead>
                  <tr>
